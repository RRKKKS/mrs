...
async def start(client, message):
   if message.from_user.username in DEVS:
     ...
     return await message.reply_text("اهلا بيف مصنع بوتات فينوم ميوزك", reply_markup=kep)
   kep = ReplyKeyboardMarkup([["صنع بوت", "حذف بوت"], ["السورس"]], resize_keyboard=True)
   await message.reply_text("اهلا بيك ف مصنع بوتات فينوم ميوزك", reply_markup=kep)

      await message.reply_video(
        video="https://t.me/B_R_R_1/2",
        caption="اهلا بك في سورس فينوم يقلبي🥺❤️",
...
async def alivehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝙶𝚁𝙾𝚄𝙿️", url=f""),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/rb_ro"),
            ],
            [
                 InlineKeyboardButton(f"DADY VENOM ", url=f"https://t.me/R_N_B1")
            ]
        ]
    )
